...
function Upgrade() {
  ...
  const spinWheel = () => {
    ...
    if (randomNumber <= winPercentage) {
      // It's a win
      const maxWinAngle = (winPercentage / 100) * 360; // Calculate max angle for win
      const randomWinAngle = Math.random() * maxWinAngle; // Random angle within win range
      setRotationAngle(-(randomWinAngle + 3 * 360)); // Set rotation angle for win
      console.log("You won" + randomWinAngle);
      setWheelColor('text-success')
    } else {
      // It's a loss
      const minLossAngle = (winPercentage / 100) * 360; // Calculate min angle for loss
      const randomLossAngle =
        minLossAngle + (Math.random() * (360 - minLossAngle)); // Random angle greater than win range
      setRotationAngle(-(randomLossAngle + 3 * 360)); // Set rotation angle for loss
      console.log("minimu: " + minLossAngle +  " You lost: " + randomLossAngle);
